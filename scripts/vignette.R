...
# Regression models
#======================================================#
glm1 <- glm(ce ~ 1 + lai_ann + rain_mm_ann + elevation_m,
...
glm.formula <- stats::formula("ce ~ poly(elevation_m,2) + poly(tair_ann, 2) + poly(rain_mm_ann,2) + lai_ann + lai_ann:rain_mm_ann + lai_ann:elevation_m + rain_mm_ann:elevation_m")
...
gg.rp <- ggplot2::ggplot(rp, aes(x = expl.val, y = pred.val, lty = pred.name)) +
  geom_line() +
  labs(y = "prob of occ", x = "") +
  #rp.gg.theme +
  facet_grid(~expl.name, scales = "free_x")
gg.rp

# do again for new models
glm3 <- glm(ce ~ poly(elevation_m, 2) + poly(rain_mm_ann, 2) + lai_ann,
            data = stats_df, family = "binomial")

glm_full <- glm(ce ~ poly(elevation_m, 2) + poly(rain_mm_ann, 2) + lai_ann + soil_moist_ann, 
                data = stats_df, family = "binomial")

# plot the response curves
rp2 <- biomod2::response.plot2(models = c("glm3", "glm_full"),
                              Data = as.data.frame(stats_df %>% dplyr::select(lai_ann, rain_mm_ann, elevation_m, tair_ann, soil_moist_ann)),
                              show.variables = c("lai_ann", "rain_mm_ann", "elevation_m",  "tair_ann", "soil_moist_ann"),
                              fixed.var.metric = "mean",
                              plot = FALSE,
                              use.formal.names = TRUE)

gg.rp2 <- ggplot2::ggplot(rp2, aes(x = expl.val, y = pred.val, lty = pred.name)) +
  geom_line() +
  labs(y = "prob of occ", x = "") +
  #rp.gg.theme +
  facet_grid(~expl.name, scales = "free_x")
...
r3 <- raster::predict(env_rs, model = glm3, progress='text', type = "response")
...
r3_pts <- raster::rasterToPoints(r3, spatial = TRUE)
...
r2_plot <- ggplot2::ggplot() +
  ...
  ggplot2::labs(title = glue::glue("glm ~ poly(elevation_m, 2) + lai_ann + rain_mm_ann AIC:{stats::AIC(glm2)}"),
                fill = "probability") +
  ggthemes::theme_map() +
  ggplot2::coord_quickmap()
r2_plot
cowplot::ggsave2(r2_plot, filename = "plots/2022-03-16_SDM_glm.png", width = 7.5, height = 5)

# plot r3df with ggplot
r3_plot <- ggplot2::ggplot() +
  #ggplot2::geom_raster(data = r2_df , aes(x = x, y = y, fill = layer)) + 
  ggplot2::geom_tile(data = r3_df , aes(x = x, y = y, fill = layer)) + 
  ggplot2::ggtitle("glm") +
  ggthemes::theme_map() +
  ggplot2::coord_quickmap()
r3_plot
...
# try making the predictions into raster object for plotting. https://rspatial.org/raster/index.html rspatial help
# Convert points to sp (assumes that the sf object is called example_points)
example_points <- as(example_points, "Spatial")
p_sp <- sp::SpatialPointsDataFrame(coords = p[, c("x", "y")], data = p, proj4string = raster::crs(env_rs[[6]]))
test <- raster::rasterize(p_sp)
...
biomod2::level.plot(fitted(glm_small),
                    XY = p_df[, c("x", "y")], #
                    color.gradient = "grey",
                    cex = 0.2,
                    show.scale = T,
...
# make a plot of predictions
biomod2::level.plot(fitted(glm1),
                    XY = as.data.frame(stats_df[, c("lon", "lat")]), #
                    color.gradient = "grey",
                    cex = 0.5,
                    show.scale = T,
