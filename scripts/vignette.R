...
# load example data
mammals_data <- read.csv("/Users/tim/repos/SDM/data/tutorial/mammals_and_bioclim_table.csv")
...
ggplot() +
  geom_raster(data = raster::as.data.frame(tair_ann_raster, xy = TRUE), 
              aes(x = x, y = y, fill = tair_ann)) + 
  #scale_fill_gradientn(name = "Elevation", colors = terrain.colors(10)) + 
  coord_quickmap()

# example plotting with raster
raster::plot(env_rs[[1:8]], col=rainbow(100,start=.0,end=.8))
